    ...
    def parse_file(file_path):
        ...
        try:
            with open(file_path, 'r') as f:
                ...
                print(f"Header for {file_path}: {header}", file=sys.stderr)
                for line in f:
                    fields = line.strip().split('\t')
                    if len(fields) != len(header):
                        print(f"Mismatch in field count for line: {line.strip()}", file=sys.stderr)
                        continue
            ...
            print(f"Parsed data for {file_path}: {json.dumps(data, indent=2)}", file=sys.stderr)
